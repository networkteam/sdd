...
func (s *FilesystemStagedBlobStore) Stage(_ context.Context, owner BlobOwner, filename string, content io.Reader) (StagedBlob, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	if filename == "" || filepath.Base(filename) != filename || strings.ContainsAny(filename, `/\\`) {
		return StagedBlob{}, fmt.Errorf("sdd: staged filename must be a plain name")
	}
...
func (s *FilesystemStagedBlobStore) Stat(_ context.Context, owner BlobOwner, id string) (StagedBlob, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.statLocked(owner, id)
}

func (s *FilesystemStagedBlobStore) statLocked(owner BlobOwner, id string) (StagedBlob, error) {
	ownerDir, err := s.ownerDir(owner)
	if err != nil {
		return StagedBlob{}, err
	}
	...
	if blob.Owner != owner || blob.ID != id {
		return StagedBlob{}, fmt.Errorf("sdd: staged blob ownership mismatch")
	}
	return blob, nil
}

func (s *FilesystemStagedBlobStore) Open(_ context.Context, owner BlobOwner, id string) (io.ReadCloser, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *FilesystemStagedBlobStore) Retain(_ context.Context, owner BlobOwner, retentionID string, ids []string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	if retentionID == "" {
...
func (s *FilesystemStagedBlobStore) Release(_ context.Context, owner BlobOwner, retentionID string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	retentions, filename, err := s.retentionsLocked(owner)
	if err != nil {
...
func (s *FilesystemStagedBlobStore) retentionsLocked(owner BlobOwner) (map[string][]string, string, error) {
	ownerDir, err := s.ownerDir(owner)
	if err != nil {
		return nil, "", err
	}
...
func (s *FilesystemStagedBlobStore) ownerDir(owner BlobOwner) (string, error) {
