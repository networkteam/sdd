...
Project B is readable as an authorized dependency.`), 0o644); err != nil {
	...
	storeA, err := sdd.NewFilesystemGraphStore(sdd.FilesystemGraphStoreOptions{Project: "project-a", GraphDir: graphA})
	if err != nil {
		t.Fatal(err)
	}
	storeB, err := sdd.NewFilesystemGraphStore(sdd.FilesystemGraphStoreOptions{Project: "project-b", GraphDir: graphB})
	if err != nil {
		t.Fatal(err)
	}
	...
	blobs, err := sdd.NewFilesystemStagedBlobStore(filepath.Join(root, "blobs"))
	if err != nil {
		t.Fatal(err)
	}
