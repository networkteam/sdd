...
func TestRefUnmarshal_LegacyAliasResolved(t *testing.T) {
	...
	// the alias lives only in the read path. The resolved ref carries no hidden
	// on-disk state, so it is == to the canonical constructed literal.
	for _, kind := range []string{"grounds", "evidence"} {
		t.Run(kind, func(t *testing.T) {
			...
			want := Ref{ID: "20260101-000000-s-cpt-aaa", Kind: RefKindGroundedIn}
			if refs[0] != want {
				t.Errorf("legacy kind %q resolved to %+v, want %+v", kind, refs[0], want)
			}
		})
	}
}

func TestRef_ParsedRefEqualsLiteral(t *testing.T) {
	// A parsed ref carries no hidden state, so it stays byte-equal (==) to its
	// constructed-literal form.
	const doc = "- id: 20260101-000000-s-cpt-aaa\n  kind: addresses\n"
	var refs []Ref
	if err := yaml.Unmarshal([]byte(doc), &refs); err != nil {
		t.Fatalf("unmarshal: %v", err)
	...
	if refs[0] != want {
		t.Errorf("parsed ref %+v must stay == to its literal form %+v", refs[0], want)
	}
}
