...
// scope is the check's design (d-cpt-20r), not a missing dependency. Returns
// an error only for infrastructure failures; findings, including none, are
// the result.
func (f *Finder) WritingGuide(ctx context.Context, q query.WritingGuideQuery) (*query.WritingGuideResult, error) {
	...
	result, err := llm.WritingGuide(ctx, f.writingGuideRunner, q.Entry)
	if err != nil {
		return nil, err
	}
