...
state:
    ...
    recognitionMode: {type: bool, default: true, desc: "the constant bootstrap seeds into every capture it dispatches — its cluster playback already approved the words, so the child's per-entry playback softens to recognition"}
steps:
    - id: orient
      inject:
...
Each entry is an ordinary capture with its own playback, confirmation, quality check, and summary verification. But the user approved these words a moment ago in the cluster playback, so the per-entry confirmation is a *recognition*, not a fresh review — the capture is dispatched with `recognitionMode` set so its playback frames lightly. Keep it that way:
...
Bootstrap is done for this run. Hand back to the ordinary working flow with a fresh read of where the graph now stands: run the **catch-up** briefing as its own move over the just-populated graph — it becomes the user's first real orientation inside their new project record. Choose **`brief`** once you have run it and said the closing words; that completes the bootstrap run.
