...
//
// SDD adaptations:
//   - mergeSplits / shouldPop / joinDocs / maybePrintWarning are inlined here
//     from upstream split_documents.go. The upstream file's SplitDocuments
//     and CreateDocuments helpers depend on github.com/tmc/langchaingo/schema
//     which we do not pull in; only these helpers are needed by the
//     recursive splitter.
...
// RecursiveCharacter is a text splitter that will split texts recursively by
...
// NewRecursiveCharacter creates a new recursive character splitter with default
// values. By default the separators are "\n\n", "\n", " " and "".
...
func maybePrintWarning(total, chunkSize int) {
	if total > chunkSize {
		log.Printf(
			"[WARN] created a chunk with size of %v, which is longer than the specified %v\n",
			total,
			chunkSize,
		)
	}
...
// shouldPop reports whether the current document accumulator should drop its
// oldest entry. Pops happen when the accumulator is larger than the overlap
// budget or when adding the next split would exceed chunkSize.
