...
- **Cache clone/pull is a command, never inside a finder.** It has side effects (git, index writes), so the CLI layer runs it before the read finders — the same shape as `cmd/sdd/search.go` calling `IndexHandler.LazyFill` before the search finder today. Finders read over already-loaded graphs/indexes and stay pure. On the capture path the handler does the fetch (side effects belong there).
- **New package `internal/repos/`** for the connected-repos registry and cache manager (clone/pull, locate a cached repo's graph + index). Side-effecting, so it sits on the command side — not in `model` (pure) or `meta` (per-repo config). Global-config loading layers under the existing `loadConfig` / `meta.ReadConfig` chain.
- **Cross-graph traversal stays pure in `model`.** The chain-walk and `(repo-id, entry-id)` dedup take a graph-lookup function as input; loading those graphs is orchestration that stays out of `model`.
- **`SearchFinder` widens from one index store to a set** keyed by repo-id; the merge-by-score stays pure. This is the main coupling change.
