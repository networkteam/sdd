...
func TestFilesystemGraphStoreRollsBackMidBatchFailure(t *testing.T) {
	...
	if err == nil || result.State != MutationNotApplied {
	...
	if err != nil || reconciled.State != MutationNotApplied || reconciled.Revision != initialRevision {
...
func TestFilesystemGraphStoreCurrentCannotObservePartialBatch(t *testing.T) {
	...
	if result := <-applyDone; result.State != MutationNotApplied {
...
func atomicGraphFixture(t *testing.T) (*FilesystemGraphStore, string, MutationBatch, map[string][]byte) {
	t.Helper()
	dir := t.TempDir()
	...
	batch := MutationBatch{ID: "atomic-mid-batch", Changes: []DocumentChange{
