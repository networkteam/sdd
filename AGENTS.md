...
- **Never rewrite or roll back git history on your own.** Do not run `git reset`, `git rebase`, `git commit --amend`, `git revert`, or a force-push without an explicit request — not even to correct a commit you just made. This repo has concurrent sessions committing to `main`, so the branch tip may not be the commit you think it is: a reset can silently undo *another session's* commit. Losing work is far worse than an imperfect commit — if a commit lands wrong (stray files, wrong message), leave it, say so plainly, and let the user decide how to fix it.
