...
- **Use Conventional Commits for every new commit not created by SDD tooling.** Format messages as `type(scope): description`, with optional scope, for example `docs: clarify commit conventions`. Reserve the `sdd` prefix exclusively for commits created by SDD tooling; never use it for manual or agent-authored commits. For merge commits, supply a conventional message with `git merge -m 'chore: merge <branch>' <branch>`. This rule applies going forward; do not rewrite existing history to conform.
