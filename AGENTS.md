...
- **Use Conventional Commits for every new commit not created by SDD tooling.** Use a lowercase type, an optional scope in parentheses, a colon and an imperative subject, for example `feat: add search synchronization` or `fix(search): verify returned candidates`. Types in use: `feat`, `fix`, `refactor`, `docs`, `test`, `chore`, `build`. Reserve the `sdd` prefix exclusively for commits created by SDD tooling; never use it for manual or agent-authored commits. Keep the body short: when the why lives in the SDD graph, reference the entry by its full ID instead of repeating the reasoning. For merge commits, supply a conventional message with `git merge -m 'chore: merge <branch>' <branch>`. This rule applies going forward; do not rewrite existing history to conform.
- **Never commit compiled binaries.** `bin/sdd` is in `.gitignore` and must stay there. Rebuild locally.
- **Stage and commit by explicit path.** Never `git add -A` or `git add .`, and always `git commit -- path/to/file …` — without a pathspec the commit records the *whole* index. Concurrent sessions and active WIP markers leave ambient changes there, and sweeping them in binds unrelated work to the wrong commit, invisible until it lands.
- **Never rewrite history; recover only by adding commits.** Every commit that exists stays, with its hash and its message — `sdd`'s own auto-commits included, since those are the record the tool produces. No reset, rebase, amend, force-push or squash, whether typed as a git command or clicked in a forge: the rule is the effect, not the spelling. Concurrent sessions commit to `main`, so the tip moves between reading the log and acting on it — even a reset to a hash you just saw can drop another session's work. To fix something, add a commit (`git cherry-pick` onto the current tip). If a commit landed wrong, leave it, say so, and let the user decide.
- **Land a branch with a plain local merge; the push is the user's.** In the main checkout: `git merge <branch>`, then say it's ready. Never land through a forge — a PR or squash merge rewrites history and puts the content on the remote under a hash the local branch never had, which is what forces a reconciliation afterwards.
