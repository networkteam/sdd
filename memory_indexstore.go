...
type MemorySearchIndexStore struct {
	mu     sync.RWMutex
	chunks map[IndexNamespace]map[string]IndexedChunk
}

func NewMemorySearchIndexStore() *MemorySearchIndexStore {
	return &MemorySearchIndexStore{chunks: map[IndexNamespace]map[string]IndexedChunk{}}
}

func (s *MemorySearchIndexStore) Manifest(_ context.Context, namespace IndexNamespace) ([]StoredChunkRef, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	items := s.chunks[namespace]
	result := make([]StoredChunkRef, 0, len(items))
	for _, item := range items {
		result = append(result, StoredChunkRef{ID: item.Chunk.ID, Revision: item.Chunk.Revision, ContentHash: item.Chunk.ContentHash})
	}
	sort.Slice(result, func(i, j int) bool { return result[i].ID < result[j].ID })
	return result, nil
}

func (s *MemorySearchIndexStore) Reconcile(_ context.Context, namespace IndexNamespace, _ string, upserts []IndexedChunk, deletes []string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.chunks[namespace] == nil {
		s.chunks[namespace] = map[string]IndexedChunk{}
	}
...
func (s *MemorySearchIndexStore) Nearest(_ context.Context, namespaces []IndexNamespace, vector []float32, limit int) ([]ScoredChunkHit, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	var result []ScoredChunkHit
	for _, namespace := range namespaces {
		...
		for _, item := range s.chunks[namespace] {
			...
			result = append(result, ScoredChunkHit{
