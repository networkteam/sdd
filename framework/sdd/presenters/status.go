...
func RenderStatus(w io.Writer, result *query.StatusResult, width int) {
	g := result.Graph
	...
	renderLayeredSection(w, "Contracts", result.Contracts, width)
	renderLayeredSection(w, "Plans", result.Plans, width)
	renderLayeredSection(w, "Active Decisions", result.Active, width)
	renderFlatSection(w, "Open Signals", result.Open, width)
	renderFlatSection(w, "Recent Actions", result.Recent, width)
}

func renderLayeredSection(w io.Writer, title string, entries []*model.Entry, width int) {
	if len(entries) == 0 {
		return
	}
	fmt.Fprintf(w, "## %s\n\n", title)
...
func renderFlatSection(w io.Writer, title string, entries []*model.Entry, width int) {
	if len(entries) == 0 {
		return
	}
	fmt.Fprintf(w, "## %s\n\n", title)
